...
<c:forEach items="${journey.destinations}" var="destination" varStatus="current">
    ...
    <div>
        Since: ${destination.since}  To: ${destination.deadline}  Cost: ${destination.cost}  <a href="${destination.link}">${destination.link}</a>
    ...
    <c:forEach var="trip" items="${destination.trips}">
            <tr>
                ...
                <td>${trip.current}</td>
                <td>
                    <a href="${pageContext.request.contextPath}/journey/destination/trip/edit?id=${trip.id}&ids=${journey.id}" class="button is-warning is-small">Edit</a>
                    <a href="${pageContext.request.contextPath}/journey/destination/trip/delete?id=${trip.id}" class="button is-danger is-small">Delete</a>
                </td>
            </tr>
        </table>
    </c:forEach>
